...
import asksage.api.models.AgentInfo
...
class AgentPanel(private val project: Project) : JPanel(BorderLayout()) {
    ...
    private val executeButton = JButton("Run Agent").apply { preferredSize = Dimension(110, 28) }
    private val refreshButton = JButton("Refresh").apply { preferredSize = Dimension(90, 28) }
    private val resultDisplay = JTextPane().apply {
        isEditable = false
        border = BorderFactory.createEmptyBorder(8, 8, 8, 8)
    }
    private val statusLabel = JLabel("Select an agent and enter a message").apply { foreground = JBColor.GRAY }
    init {
        setupUI()
        setupActions()
        loadData()
    ...
    private fun setupUI() {
        ...
        val inputPanel = JPanel(BorderLayout()).apply {
            ...
            add(JPanel().apply { layout = BoxLayout(this, BoxLayout.Y_AXIS); border = BorderFactory.createEmptyBorder(4, 4, 4, 4); add(executeButton) }, BorderLayout.EAST)
        }
    ...
    private fun executeAgent() {
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            try {
                ...
                val request = ExecuteAgentRequest(agentId = agentId, message = message)
    ...
    companion object { private val LOG = logger<AgentPanel>() }
}
